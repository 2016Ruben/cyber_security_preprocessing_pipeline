...
b_size=1
model_type="kitnet" # "vanilla
input_type="kitsune_original"
output_dir="results_kitnet"
...
for dir in data/kitsune_data/*/; do
  ...
  pcapf="${dir}${dsname}_pcap.pcapng.tsv"
  ...
  for it in {1..1}; do
    ...
    { time python main.py ${pcapf} ${yaml_input} --input_type=${input_type} --model_type=${model_type} --labelf_path=${labelf} --ngram_size=${ngram_size} --save_scaler=0 --figure_path=${figure_file} --model_save_path=${model_file} --results_dict_save_path=${res_file} --b_size=${b_size} --n_training_examples=1000000 ; } 2> ${time_file}
  done
done